...
class Users extends React.Component {
    ...
    render() {
        ...
        /*if ((this.props.currentPage>3) &&(this.props.currentPage<(pagesCount-3))) {
            pages = [1, this.props.currentPage-1, this.props.currentPage, this.props.currentPage+1, pagesCount]
        }*/

        for (let i = 1; i <= this.props.currentPage + 1; i++) {
            pages.push(i)
        }
        ...
        pages = slicedPages.map(page => {
            return <span
                className={((this.props.currentPage === page) && styles.selectedUsersPage) + ' ' + styles.pageItem}
                onClick={(e) => {
        ...
        const allUsers = this.props.usersData.map(usersItem => {
            return <div className={styles.user_item}>
                <img src={(usersItem.photos.small!=null)?usersItem.photos.small:avatarForAll} className={styles.ava} alt='Аватар'/>
